...
export function generateProvidedPassageV02Prompt(set: EnglishQuestionSet) {
  ...
  const grammar = `${grammarDetails || '- 어법 문항 없음'}\n- Request Schema V0.2의 items[].requiredStem은 additional property가 아니라 item.required와 item.properties에 모두 정의된 정식 필수 필드다. 이를 오류로 판정하거나 Request에서 삭제하지 않는다.\n- 승인 후 1차 JSON은 문제지와 정답지 완성이 목적이다. question의 explanation, intention, distractorReasons와 item의 qualityReview는 출력하지 않는다. evidenceSpans와 materialOperation은 구조 검증을 위해 반환한다.\n- 앞의 해설 작성 지시는 2차 해설 생성 단계에만 적용하며, 1차 JSON에서는 해설 필드를 생략한다.\n- 기존 방식대로 해설 필드와 qualityReview를 함께 출력해도 앱은 호환하여 가져올 수 있다.`
