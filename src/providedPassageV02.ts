...
export function generateProvidedPassageV02Prompt(set: EnglishQuestionSet) {
  return generateProvidedPassageV02PromptRaw(set)
    .replace(
      '- 모든 items는 동일한 source.passage를 근거로 한다.',
      '- 모든 items는 동일한 source.passage 한 지문을 공유해 근거로 삼는다.',
    )
    .replace(
...
export function providedPassageV02SharedMaterialText(set: EnglishQuestionSet) {
  const state = set.providedPassageV02
  const hasRegularQuestion = orderedProvidedPassageV02Questions(set).some((question) => question.type !== '문장 삽입' && question.type !== '요약문 완성')
  if (!state || !hasRegularQuestion) return undefined
