...
export interface ProblemProject {
  id: string; title: string; subject: Subject; grade: string; curriculum: string; unit: string; topic: string; difficulty: number; questionType: 'multiple' | 'short';
  idea: string; traits: string[]; answerPreference: string; avoidTags: string[]; requiredElement: string; reference: string;
  calculationScope: string; koreanMode: 'provided' | 'generated'; passageTopic: string; passageLength: 'short' | 'medium' | 'long';
...
export const emptyProject = (subject: Subject = 'math'): ProblemProject => ({
  id: crypto.randomUUID(), title: subject === 'math' ? '새 수학 문제' : '새 국어 문제', subject, grade: '고등학교', curriculum: '', unit: '', topic: '', difficulty: 3, questionType: 'multiple', idea: '', traits: [], answerPreference: '정수 선호', avoidTags: [], requiredElement: '', reference: '', calculationScope: '', koreanMode: 'provided', passageTopic: '', passageLength: 'medium', passage: '', abilities: [], distractorTypes: [], questionStyle: '평가원형', generatedPrompt: '', problem: '', choices: ['', '', '', '', ''], answer: '', solution: '', intention: '', memo: '', reviewResults: [], reviewChecklist: {}, qualityRatings: { satisfaction: 3, novelty: 3, neatness: 3, thinking: 3, naturalness: 3, difficultyFit: 3 }, likes: '', dislikes: '', status: 'idea', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
})
