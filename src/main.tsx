  ...
  const importBackup = (file?: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        notify('영어 전용 백업을 이 기기에 복원했습니다.')
      } catch (error) { notify(error instanceof Error ? error.message : '백업 파일을 읽지 못했습니다.') }
    }
    reader.readAsText(file)
  }
  return <main className="app-shell">
    <header className="app-header"><div className="brand"><span className="brand-mark">E</span><div><span className="eyebrow">ENGLISH QUESTION DESIGN WORKBENCH</span><h1>영어 문제 제작 연구소</h1></div></div><div className="header-actions"><span className="sync-badge synced">이 기기에 자동 저장</span><button onClick={exportBackup}>JSON 백업</button><button onClick={() => importRef.current?.click()}>백업 복원</button><input ref={importRef} type="file" accept="application/json" hidden onChange={(event) => { importBackup(event.target.files?.[0]); event.currentTarget.value = '' }} /></div></header>
