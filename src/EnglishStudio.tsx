...
import { orderedSchoolQuestions, schoolInlineChoiceLabels, usesInlineSchoolChoices } from './schoolMaterial'
  ...
  const patchSchoolProvidedV02Plan = (itemId: string, patch: Partial<ProvidedPassageV02ItemPlan>) => {
    ...
    const plans = schoolProvidedV02.itemPlans.map((plan) => {
      ...
      if (patch.questionType) {
        next.choiceLanguage = createProvidedPassageV02Plan(plan.itemId, patch.questionType).choiceLanguage
          ...
          {mode === 'school' && schoolProvidedV02 ? <section className="provided-passage-panel" data-testid="school-provided-passage-panel-v02">
            ...
            <div className="provided-plan-list">{schoolProvidedV02Plans.map((plan, index) => <article className="provided-plan-card" key={plan.itemId}>
              ...
              <div className="form-grid"><label>문항 유형<select value={plan.questionType} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { questionType: event.target.value as ProvidedPassageV02QuestionType })}>{(Object.keys(PROVIDED_PASSAGE_V02_TYPE_LABELS) as ProvidedPassageV02QuestionType[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_V02_TYPE_LABELS[value]}</option>)}</select><small className="field-help">유형·발문은 V0.2 계약이 함께 관리합니다. 삽입 위치 표식은 해당 문항에만 적용됩니다.</small></label>
                <label>발문 언어<small className="field-help">유형별 기본 발문을 한국어 또는 영어로 출력합니다.</small><select value={plan.stemLanguage ?? 'ko'} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { stemLanguage: event.target.value as ProvidedPassageChoiceLanguage })}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label>
                {(plan.questionType === 'content_match' || plan.questionType === 'content_inference' || plan.questionType === 'summary') && <label>선지 언어<select value={plan.choiceLanguage ?? (plan.questionType === 'summary' ? 'en' : 'ko')} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { choiceLanguage: event.target.value as ProvidedPassageChoiceLanguage })}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label>}
                ...
                {plan.questionType === 'summary' && <div className="grammar-plan-guidance"><strong>요약문 완성 구조</strong><span>공통 지문을 재진술한 영어 요약문에 (A)·(B) 두 빈칸을 두고, 다섯 개의 영어 단어쌍 중 하나를 고릅니다.</span><small>AI 결과의 summaryText에는 [[요약빈칸:A]]와 [[요약빈칸:B]]가 각각 한 번 필요합니다.</small></div>}
...
function QuestionEditor({ set, question, index, locked = false, updateQuestion, remove }: { set: EnglishQuestionSet; question: EnglishQuestion; index: number; locked?: boolean; updateQuestion: (id: string, patch: Partial<EnglishQuestion>) => void; remove: () => void }) {
  ...
  const languageAwareChoices = set.mode === 'school' && schoolQuestionUsesChoiceLanguage(question)
  ...
  return <details className="question-editor editor-fold">
    ...
    <div className="editor-fold-body">
      <div className="form-grid"><label>문항 유형<small className="field-help">학생에게 요구할 풀이 방식과 문제 구조를 정합니다. 유형을 바꾸면 기본 발문과 선지 조판도 함께 바뀝니다.</small><select value={question.type} disabled={locked} onChange={(event) => { const type = event.target.value; const template = set.mode === 'school' ? SCHOOL_QUESTION_TEMPLATES.find((candidate) => candidate.questionType === type) : undefined; const stemLanguage = question.schoolStemLanguage ?? 'ko'; updateQuestion(question.id, { type, stem: defaultQuestionStem(type, stemLanguage), schoolTemplateId: template?.id, schoolChoiceLayout: template?.choiceLayout, schoolStemLanguage: stemLanguage, schoolChoiceLanguage: template?.id === 'summary' ? question.schoolChoiceLanguage ?? 'en' : question.schoolChoiceLanguage ?? 'ko', schoolSummaryText: template?.id === 'summary' ? question.schoolSummaryText ?? '' : undefined }) }}>{locked ? <option>{question.type}</option> : questionTypesFor(set.mode).map((type) => <option key={type}>{type}</option>)}</select></label><label>배점<small className="field-help">시험지에 표시할 문항 점수입니다. 소수 배점도 사용할 수 있습니다.</small><input type="number" min="0.1" max="100" step="0.1" value={question.score ?? 2} onChange={(event) => updateQuestion(question.id, { score: Number(event.target.value) })} /></label>{set.mode === 'school' && <label>선지 표시<small className="field-help">자동은 선지 길이에 따라 한 줄·세로형을 결정합니다. 조합형은 표 형태가 적합합니다.</small><select value={question.schoolChoiceLayout ?? inferSchoolQuestionTemplate(question).choiceLayout} onChange={(event) => updateQuestion(question.id, { schoolChoiceLayout: event.target.value as EnglishQuestion['schoolChoiceLayout'] })}><option value="auto">길이에 따라 자동</option><option value="inline">짧은 선지 · 한 줄/복수 열</option><option value="vertical">긴 선지 · 세로형</option><option value="matrix">조합형 · 표 형태</option></select></label>}</div>
      {set.mode === 'school' && !locked && <div className="form-grid"><label>발문 언어<small className="field-help">영어를 선택하면 유형에 맞는 기본 영어 발문으로 바뀝니다. 이후 직접 수정할 수도 있습니다.</small><select value={question.schoolStemLanguage ?? 'ko'} onChange={(event) => { const language = event.target.value as ProvidedPassageChoiceLanguage; updateQuestion(question.id, { schoolStemLanguage: language, stem: defaultQuestionStem(question.type, language) }) }}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label><label>선지 언어<small className="field-help">다음 AI 생성에서 선지 본문을 선택한 언어로 통일합니다. 위치 번호·배열형 문항에는 적용되지 않습니다.</small><select disabled={!languageAwareChoices} value={question.schoolChoiceLanguage ?? (inferSchoolQuestionTemplate(question).id === 'summary' ? 'en' : 'ko')} onChange={(event) => updateQuestion(question.id, { schoolChoiceLanguage: event.target.value as ProvidedPassageChoiceLanguage })}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label></div>}
