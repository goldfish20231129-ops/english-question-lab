...
function SetWorkspace({ mode, bundle, setBundle, notify, onOpenVerification }: Props) {
  ...
  const patchSchoolProvidedV02Plan = (itemId: string, patch: Partial<ProvidedPassageV02ItemPlan>) => {
    ...
    const plans = schoolProvidedV02.itemPlans.map((plan) => {
      ...
      if (patch.stemLanguage) next.choiceLanguage = usesProvidedPassageV02ChoiceLanguage(next.questionType) ? patch.stemLanguage : null
      if (patch.questionType) {
        next.choiceLanguage = usesProvidedPassageV02ChoiceLanguage(patch.questionType) ? next.stemLanguage ?? 'ko' : null
  ...
  const addSchoolProvidedV02Plan = (questionType: ProvidedPassageV02QuestionType = 'content_match') => {
    ...
    if (questionType === 'grammar') { nextPlan.grammarTarget = null; nextPlan.grammarMode = 'controlled_error_variant' }
          ...
          {mode === 'school' && schoolProvidedV02 ? <section className="provided-passage-panel" data-testid="school-provided-passage-panel-v02">
            ...
            <div className="provided-plan-list">{schoolProvidedV02Plans.map((plan, index) => <article className="provided-plan-card" key={plan.itemId}>
              ...
              <div className="form-grid"><label>문항 유형<select value={plan.questionType} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { questionType: event.target.value as ProvidedPassageV02QuestionType })}>{(Object.keys(PROVIDED_PASSAGE_V02_TYPE_LABELS) as ProvidedPassageV02QuestionType[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_V02_TYPE_LABELS[value]}</option>)}</select><small className="field-help">유형·발문은 V0.2 계약이 함께 관리합니다. 삽입 위치 표식은 해당 문항에만 적용됩니다.</small></label>
                <label>문항·선지 언어<small className="field-help">발문과 내용 선지를 선택한 언어로 함께 통일합니다. 위치 기호는 언어와 관계없이 유지됩니다.</small><select value={plan.stemLanguage ?? 'ko'} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { stemLanguage: event.target.value as ProvidedPassageChoiceLanguage })}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label>
              ...
              {plan.questionType === 'grammar' && <div className="grammar-plan-guidance"><strong>{plan.grammarTarget ? `${PROVIDED_PASSAGE_GRAMMAR_LABELS[plan.grammarTarget]} 판정 기준` : PROVIDED_PASSAGE_GRAMMAR_AUTO_LABEL}</strong><span>{plan.grammarTarget ? PROVIDED_PASSAGE_GRAMMAR_RULES[plan.grammarTarget] : PROVIDED_PASSAGE_GRAMMAR_AUTO_RULE}</span><small>{PROVIDED_PASSAGE_GRAMMAR_MODE_HELP[plan.grammarMode ?? 'controlled_error_variant']} 평가원형에서는 관계사·동격 that·수 일치·동사/준동사·능수동·대명사 등 판정 가능한 핵심 항목을 분산해 지문 안에 서로 구분되는 다섯 기호와 밑줄로 표시합니다.</small>{plan.grammarMode === 'source_form_check' && <button type="button" onClick={() => patchSchoolProvidedV02Plan(plan.itemId, { grammarMode: 'controlled_error_variant' })}>평가원형 5개 밑줄로 전환</button>}</div>}
            </article>)}</div>
