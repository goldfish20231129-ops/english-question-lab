...
function QuestionEditor({ set, question, index, locked = false, updateQuestion, remove }: { set: EnglishQuestionSet; question: EnglishQuestion; index: number; locked?: boolean; updateQuestion: (id: string, patch: Partial<EnglishQuestion>) => void; remove: () => void }) {
  ...
  return <details className="question-editor editor-fold">
    ...
    <div className="editor-fold-body">
      ...
      <label>상세 해설<small className="field-help">정답이 되는 이유와 필요한 독해 과정을 설명합니다. 삽입 위치는 ①~⑤로 표시됩니다.</small><textarea value={displayText(question.explanation)} onChange={(event) => updateQuestion(question.id, { explanation: event.target.value })} /></label>
      <div className="form-grid"><label>정답 근거 <small className="field-help">지문에서 정답을 확정하는 문장이나 정보를 한 줄에 하나씩 적습니다.</small><textarea value={displayText(question.evidenceRefs.join('\n'))} onChange={(event) => updateQuestion(question.id, { evidenceRefs: event.target.value.split('\n').map((item) => item.trim()).filter(Boolean) })} /></label><label>오답 오류 근거 <small className="field-help">각 오답이 틀린 이유를 선지 순서에 맞춰 한 줄씩 적습니다. 삽입 위치는 ①~⑤로 표시됩니다.</small><textarea value={displayText(question.distractorReasons.join('\n'))} onChange={(event) => updateQuestion(question.id, { distractorReasons: event.target.value.split('\n').map((item) => item.trim()).filter(Boolean) })} /></label></div>
    </div>
  </details>
}
