...
import { CUSTOM_PRESETS, ENGLISH_INTENTION_PRESETS, ENGLISH_TOPIC_PRESETS, LAYOUT_PRESETS, MODE_LABELS, applyCustomPreset, assignAutomaticCsatTopics, createEnglishSet, createExamLayout, createQuestion, generateCsatGptInstructions, generateEnglishPrompt, generateReviewPrompt, layoutForFirstSelectedSet, loadEnglishGptConfig, parseEnglishSetJson, preferredExamPresetForSets, questionTypesFor, validateEnglishSet, type EnglishGptConfig } from './english'
...
function SetWorkspace({ mode, bundle, setBundle, notify, onOpenVerification }: Props) {
  ...
  const schoolProvided = active?.mode === 'school' ? active.providedPassage : undefined
  const hasProvidedPassage = Boolean(schoolProvided)
  const schoolProvidedBlockingReason = active && schoolProvided ? providedPassageBlockingReason(active) : undefined
  ...
  const selectSchoolPassageMode = (nextMode: 'provided' | 'generated') => {
    ...
    updateSet(transitionSchoolProvidedPassageMode(active, nextMode))
  }
  ...
  const updateSchoolProvidedMaterial = (material: string) => {
    if (!schoolProvided) return
    updateSet({ material, providedPassage: updateProvidedPassageState(schoolProvided, material), prompt: '', lastImportedJson: '' })
  }
      ...
      <section className="set-editor">
        ...
        <section className="editor-card">
          ...
          <div className="form-grid">
            ...
            {mode === 'school' && schoolProvided && <label>자료 출처<select value={active.sourceKind} onChange={(event) => updateSet({ sourceKind: event.target.value as EnglishQuestionSet['sourceKind'] })}><option value="textbook">교과서 본문</option><option value="supplement">부교재 지문</option><option value="external">외부 지문</option></select></label>}
          ...
          {mode === 'school' && <section className={`passage-source-mode ${schoolProvided ? 'provided' : 'generated'}`} data-testid="school-passage-source-mode"><div><strong>자료 작성 방식</strong><small>{schoolProvided ? '사용자가 입력한 원문은 앱이 보존하고 AI는 문항만 만듭니다.' : 'AI가 새 자료와 문항을 함께 작성하는 기존 방식입니다.'}</small></div><label><select aria-label="내신형 자료 작성 방식" value={schoolProvided ? 'provided' : 'generated'} onChange={(event) => selectSchoolPassageMode(event.target.value as 'provided' | 'generated')}><option value="generated">새 자료 작성</option><option value="provided">기존 지문 사용</option></select></label></section>}
          {mode === 'school' && schoolProvided ? <section className="provided-passage-panel" data-testid="school-provided-passage-panel">
        ...
        {mode === 'csat' ? <CsatItemsEditor set={normalizeCsatSet(active)} items={getCsatItems(active)} updateItems={updateCsatItems} assets={bundle.mediaAssets.filter((asset) => asset.setId === active.id)} uploadImage={uploadImage} updateAsset={updateAsset} removeAsset={removeAsset} notify={notify} /> : <section className="editor-card">
          <div className="card-title-row"><div><h3>2. 문항 유형과 문항 수</h3><p>{schoolProvided ? '기존 지문 V0.1은 내용 일치·불일치 또는 문장 삽입 한 문항을 생성합니다.' : '발문·선지를 비워 두면 외부 AI가 조건에 맞춰 완성합니다.'}</p></div><button disabled={Boolean(schoolProvided)} onClick={() => updateSet({ questions: [...active.questions, createQuestion(questionTypesFor(mode)[0], active.choiceCount)] })}>+ 문항 추가</button></div>
          <div className="question-editor-list">{active.questions.map((question, index) => <QuestionEditor key={question.id} set={active} question={question} index={index} locked={Boolean(schoolProvided)} updateQuestion={updateQuestion} remove={() => updateSet({ questions: active.questions.filter((item) => item.id !== question.id) })} />)}</div>
        </section>}
...
function CsatItemCard({ set, item, index, total, change, move, duplicate, remove, assets, uploadImage, updateAsset, removeAsset }: { set: EnglishQuestionSet; item: CsatItemDesign; index: number; total: number; change: (patch: Partial<CsatItemDesign>) => void; move: (direction: -1 | 1) => void; duplicate: () => void; remove: () => void; assets: MediaAsset[]; uploadImage: (file?: File) => void; updateAsset: (asset: MediaAsset) => void; removeAsset: (asset: MediaAsset) => void }) {
  ...
  return <details className="csat-item-card" open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
    ...
    <div className="csat-item-body">
      ...
      {design && template && <>
        <div className="item-override-panel"><h4>세트 공통값 덮어쓰기</h4><p className="hint">비워 두면 세트 공통 조건을 사용하며, 입력한 항목만 이 카드에 따로 적용됩니다.</p><div className="form-grid"><label>대상 수준<small className="field-help">이 문항만 다른 학년이나 실력대로 조정할 때 입력합니다.</small><input value={item.targetLevel ?? ''} placeholder={`공통값: ${set.targetLevel}`} onChange={(event) => change({ targetLevel: event.target.value })} /></label><label>난이도<small className="field-help">이 문항만 세트 기본 난이도와 다르게 설정합니다.</small><select value={item.difficulty ?? ''} onChange={(event) => change({ difficulty: event.target.value ? Number(event.target.value) : undefined })}><option value="">공통값 ({set.difficulty}/5)</option>{[1, 2, 3, 4, 5].map((value) => <option value={value} key={value}>{value}/5</option>)}</select></label><QuickPresetField label="주제·소재" help="이 문항 지문의 중심 분야입니다. 비워 두면 공통값 또는 자동 배정값을 사용합니다." value={item.topic ?? ''} choices={ENGLISH_TOPIC_PRESETS} placeholder={set.topic ? `공통값: ${set.topic}` : '미입력 시 프롬프트 생성 때 자동 배정'} onChange={(topic) => change({ topic })} /><QuickPresetField label="출제 의도" help="이 문항에서 특별히 평가할 추론·이해 능력을 지정합니다." value={item.intention ?? ''} choices={ENGLISH_INTENTION_PRESETS} multiline placeholder={`공통값: ${set.intention || '유형에 맞게 설정'}`} onChange={(intention) => change({ intention })} /></div></div>
...
function QuestionEditor({ set, question, index, locked = false, updateQuestion, remove }: { set: EnglishQuestionSet; question: EnglishQuestion; index: number; locked?: boolean; updateQuestion: (id: string, patch: Partial<EnglishQuestion>) => void; remove: () => void }) {
  ...
  return <details className="question-editor editor-fold">
    ...
    <div className="editor-fold-body">
      <div className="form-grid"><label>문항 유형<small className="field-help">학생에게 요구할 풀이 방식과 문제 구조를 정합니다.</small><select value={question.type} disabled={locked} onChange={(event) => updateQuestion(question.id, { type: event.target.value })}>{locked ? <option>{question.type}</option> : questionTypesFor(set.mode).map((type) => <option key={type}>{type}</option>)}</select></label><label>배점<small className="field-help">시험지에 표시할 문항 점수입니다.</small><input type="number" min="1" max="10" value={question.score ?? 2} onChange={(event) => updateQuestion(question.id, { score: Number(event.target.value) })} /></label></div>
