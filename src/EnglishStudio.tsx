...
import { CUSTOM_PRESETS, ENGLISH_INTENTION_PRESETS, ENGLISH_TARGET_LEVELS, ENGLISH_TOPIC_PRESETS, LAYOUT_PRESETS, MODE_LABELS, applyCustomPreset, assignAutomaticCsatTopics, createEnglishSet, createExamLayout, createQuestion, defaultQuestionStem, generateCsatGptInstructions, generateEnglishPrompt, generateReviewPrompt, layoutForFirstSelectedSet, loadEnglishGptConfig, normalizeEnglishTargetLevel, parseEnglishSetJson, preferredExamPresetForSets, questionTypesFor, validateEnglishSet, type EnglishGptConfig } from './english'
...
import { PROVIDED_PASSAGE_GRAMMAR_AUTO_LABEL, PROVIDED_PASSAGE_GRAMMAR_AUTO_RULE, PROVIDED_PASSAGE_GRAMMAR_DESIGN_PROFILE_LABELS, PROVIDED_PASSAGE_GRAMMAR_DESIGN_PROFILE_RULES, PROVIDED_PASSAGE_GRAMMAR_LABELS, PROVIDED_PASSAGE_GRAMMAR_MODE_HELP, PROVIDED_PASSAGE_GRAMMAR_MODE_LABELS, PROVIDED_PASSAGE_GRAMMAR_RULES, PROVIDED_PASSAGE_V02_MAX_ITEMS, PROVIDED_PASSAGE_V02_TYPE_LABELS, createProvidedPassageV02Plan, humanizeProvidedPassageBoundaryReferences, orderedProvidedPassageV02Plans, providedPassageV02BlockingReason, providedPassageV02DefaultStem, providedPassageV02TransitionBlockingReason, repairProvidedPassageV02QuestionStems, syncProvidedPassageV02Questions, transitionSchoolProvidedPassageV02, updateProvidedPassageV02Material, usesProvidedPassageV02ChoiceLanguage } from './providedPassageV02'
...
import { orderedSchoolQuestions, schoolInlineChoiceLabels, schoolQuestionDisplayStem, usesInlineSchoolChoices } from './schoolMaterial'
import { MAX_SCHOOL_SET_QUESTIONS, SCHOOL_QUESTION_TEMPLATES, inferSchoolQuestionTemplate, schoolQuestionUsesChoiceLanguage } from './schoolCatalog'
...
function SetWorkspace({ mode, bundle, setBundle, notify, onOpenVerification }: Props) {
  ...
  const patchSchoolProvidedV02Plan = (itemId: string, patch: Partial<ProvidedPassageV02ItemPlan>) => {
    ...
    const plans = schoolProvidedV02.itemPlans.map((plan) => {
      ...
      if (patch.stemLanguage) next.choiceLanguage = next.questionType === 'summary' ? 'en' : usesProvidedPassageV02ChoiceLanguage(next.questionType) ? patch.stemLanguage : null
      if (patch.questionType) {
        next.choiceLanguage = patch.questionType === 'summary' ? 'en' : usesProvidedPassageV02ChoiceLanguage(patch.questionType) ? next.stemLanguage ?? 'ko' : null
        next.contentMatchPolarity = patch.questionType === 'content_match' ? plan.contentMatchPolarity ?? 'mismatch' : null
        next.grammarTarget = patch.questionType === 'grammar' ? (plan.questionType === 'grammar' ? plan.grammarTarget : null) : null
        next.grammarMode = patch.questionType === 'grammar' ? plan.grammarMode ?? 'controlled_error_variant' : null
        next.grammarDesignProfile = patch.questionType === 'grammar' ? plan.grammarDesignProfile ?? 'school_exam_balanced' : null
      }
      return next
    })
  ...
  const addSchoolProvidedV02Plan = (questionType: ProvidedPassageV02QuestionType = 'content_match') => {
    ...
    if (questionType === 'grammar') { nextPlan.grammarTarget = null; nextPlan.grammarMode = 'controlled_error_variant'; nextPlan.grammarDesignProfile = 'school_exam_balanced' }
  ...
  const updateSchoolProvidedOptions = (patch: Partial<Pick<NonNullable<EnglishQuestionSet['providedPassage']>, 'choiceLanguage' | 'vocabularyLevel' | 'contentMatchPolarity'>>) => {
    ...
    const language = nextState.choiceLanguage
      ...
      <section className="set-editor">
        ...
        <section className="editor-card">
          ...
          <div className="form-grid">
            ...
            <label>대상 수준<small className="field-help">문제를 풀 학생의 학년을 고1·고2·고3 중에서 선택합니다.</small><select value={normalizeEnglishTargetLevel(active.targetLevel, active.mode)} onChange={(event) => updateSet({ targetLevel: event.target.value })}>{ENGLISH_TARGET_LEVELS.map((level) => <option value={level} key={level}>{level}</option>)}</select></label>
            {mode === 'custom'
          ...
          {mode === 'school' && schoolProvidedV02 ? <section className="provided-passage-panel" data-testid="school-provided-passage-panel-v02">
            ...
            <div className="provided-plan-list">{schoolProvidedV02Plans.map((plan, index) => <article className="provided-plan-card" key={plan.itemId}>
              ...
              <div className="form-grid"><label>문항 유형<select value={plan.questionType} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { questionType: event.target.value as ProvidedPassageV02QuestionType })}>{(Object.keys(PROVIDED_PASSAGE_V02_TYPE_LABELS) as ProvidedPassageV02QuestionType[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_V02_TYPE_LABELS[value]}</option>)}</select><small className="field-help">유형·발문은 V0.2 계약이 함께 관리합니다. 삽입 위치 표식은 해당 문항에만 적용됩니다.</small></label>
                <label>{plan.questionType === 'summary' ? '발문 언어' : '문항·선지 언어'}<small className="field-help">{plan.questionType === 'summary' ? '발문 언어만 선택합니다. 요약문과 A·B 단어쌍은 영어로 고정됩니다.' : '발문과 내용 선지를 선택한 언어로 함께 통일합니다. 위치 기호는 언어와 관계없이 유지됩니다.'}</small><select value={plan.stemLanguage ?? 'ko'} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { stemLanguage: event.target.value as ProvidedPassageChoiceLanguage })}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label>
                ...
                {plan.questionType === 'summary' && <div className="grammar-plan-guidance"><strong>요약문 완성 구조</strong><span>독립 블록에 원문을 다시 출력하고, 영어 요약문과 영어 (A)·(B) 단어쌍 다섯 개를 표시합니다.</span><small>AI 결과의 summaryText에는 [[요약빈칸:A]]와 [[요약빈칸:B]]가 각각 한 번 필요합니다.</small></div>}
                {plan.questionType === 'grammar' && <><label>어법 설계 프로필<small className="field-help">2025 고1 학교 시험 어법 13문항의 구조 관찰을 반영한 선호값입니다. 원문 근거가 우선이며 없는 문법을 강제하지 않습니다.</small><select aria-label="어법 설계 프로필" value={plan.grammarDesignProfile ?? 'school_exam_balanced'} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { grammarDesignProfile: event.target.value as ProvidedPassageGrammarDesignProfile })}>{(Object.keys(PROVIDED_PASSAGE_GRAMMAR_DESIGN_PROFILE_LABELS) as ProvidedPassageGrammarDesignProfile[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_GRAMMAR_DESIGN_PROFILE_LABELS[value]}</option>)}</select></label><label>우선 문법<small className="field-help">내신형 5개 밑줄형에서는 선택한 문법이 원문에 없으면 다른 판정 가능한 문법으로 자동 전환합니다. 자동 선택이 가장 안전합니다.</small><select value={plan.grammarTarget ?? 'auto'} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { grammarTarget: event.target.value === 'auto' ? null : event.target.value as ProvidedPassageGrammarTarget })}><option value="auto">{PROVIDED_PASSAGE_GRAMMAR_AUTO_LABEL}</option>{(Object.keys(PROVIDED_PASSAGE_GRAMMAR_LABELS) as ProvidedPassageGrammarTarget[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_GRAMMAR_LABELS[value]}</option>)}</select></label><label>출제 방식<select value={plan.grammarMode ?? 'controlled_error_variant'} onChange={(event) => patchSchoolProvidedV02Plan(plan.itemId, { grammarMode: event.target.value as ProvidedPassageGrammarMode })}>{(Object.keys(PROVIDED_PASSAGE_GRAMMAR_MODE_LABELS) as ProvidedPassageGrammarMode[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_GRAMMAR_MODE_LABELS[value]}</option>)}</select></label></>}
              ...
              {plan.questionType === 'grammar' && <div className="grammar-plan-guidance"><strong>{PROVIDED_PASSAGE_GRAMMAR_DESIGN_PROFILE_LABELS[plan.grammarDesignProfile ?? 'school_exam_balanced']}</strong><span>{PROVIDED_PASSAGE_GRAMMAR_DESIGN_PROFILE_RULES[plan.grammarDesignProfile ?? 'school_exam_balanced']}</span><strong>{plan.grammarTarget ? `${PROVIDED_PASSAGE_GRAMMAR_LABELS[plan.grammarTarget]} 판정 기준` : PROVIDED_PASSAGE_GRAMMAR_AUTO_LABEL}</strong><span>{plan.grammarTarget ? PROVIDED_PASSAGE_GRAMMAR_RULES[plan.grammarTarget] : PROVIDED_PASSAGE_GRAMMAR_AUTO_RULE}</span><small>{PROVIDED_PASSAGE_GRAMMAR_MODE_HELP[plan.grammarMode ?? 'controlled_error_variant']} 실제 내신 시험지처럼 관계사·수 일치·동사/준동사·능수동·대명사 등 판정 가능한 핵심 항목을 분산하고, 단어 또는 구의 정확한 범위에 기호와 밑줄을 표시합니다.</small>{plan.grammarMode === 'source_form_check' && <button type="button" onClick={() => patchSchoolProvidedV02Plan(plan.itemId, { grammarMode: 'controlled_error_variant' })}>내신형 5개 밑줄로 전환</button>}</div>}
            </article>)}</div>
            ...
            <div className="form-grid"><label>문항 유형<select value={schoolProvided.questionType} onChange={(event) => selectSchoolProvidedQuestionType(event.target.value as 'content_match' | 'sentence_insertion')}><option value="content_match">내용 일치·불일치</option><option value="sentence_insertion">문장 삽입</option></select></label><label>문항·선지 언어<small className="field-help">발문과 내용 선지를 선택한 언어로 함께 통일합니다. 위치 기호는 그대로 유지됩니다.</small><select aria-label="내신형 문항·선지 언어" value={schoolProvided.choiceLanguage} onChange={(event) => updateSchoolProvidedOptions({ choiceLanguage: event.target.value as ProvidedPassageChoiceLanguage })}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option value={value} key={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label><label>생성 영어 어휘 수준<small className="field-help">원문이 아니라 새 영어 선지·삽입 문장에만 적용합니다.</small><select value={schoolProvided.vocabularyLevel} onChange={(event) => updateSchoolProvidedOptions({ vocabularyLevel: event.target.value as ProvidedPassageVocabularyLevel })}>{(Object.keys(PROVIDED_PASSAGE_VOCABULARY_LABELS) as ProvidedPassageVocabularyLevel[]).map((value) => <option value={value} key={value}>{PROVIDED_PASSAGE_VOCABULARY_LABELS[value]}</option>)}</select></label><label>문항 수<small className="field-help">V0.1 MVP는 기존 지문당 한 문항만 지원합니다.</small><select aria-label="내신형 기존 지문 문항 수" value="1" disabled><option value="1">1문항</option></select></label>{schoolProvided.questionType === 'content_match' && <label>내용 일치 발문 극성<select value={schoolProvided.contentMatchPolarity} onChange={(event) => updateSchoolProvidedOptions({ contentMatchPolarity: event.target.value as ProvidedPassageContentPolarity })}>{(Object.keys(PROVIDED_PASSAGE_POLARITY_LABELS) as ProvidedPassageContentPolarity[]).map((value) => <option value={value} key={value}>{PROVIDED_PASSAGE_POLARITY_LABELS[value]}</option>)}</select></label>}</div>
            <label>영어 지문 입력 (필수)<textarea aria-label="내신형 영어 지문 입력" className="material-input" value={active.material} onChange={(event) => updateSchoolProvidedMaterial(event.target.value)} placeholder="수정 없이 내신 문항의 근거로 사용할 영어 원문을 붙여넣으세요." /></label>
        ...
        {mode === 'csat' ? <CsatItemsEditor set={normalizeCsatSet(active)} items={getCsatItems(active)} updateItems={updateCsatItems} assets={bundle.mediaAssets.filter((asset) => asset.setId === active.id)} uploadImage={uploadImage} updateAsset={updateAsset} removeAsset={removeAsset} notify={notify} /> : <section className="editor-card">
          ...
          {mode === 'school' && (!hasProvidedPassage || Boolean(schoolProvidedV02)) && <div className="school-insertion-presentation"><strong>특수 문항 지문 표시</strong><small className="field-help">일반 문항은 공통 지문을 한 번만 사용하고, 요약문 완성과 문장 삽입은 같은 원문을 각 문항에 다시 출력합니다.</small></div>}
...
function QuestionEditor({ set, question, index, locked = false, updateQuestion, remove }: { set: EnglishQuestionSet; question: EnglishQuestion; index: number; locked?: boolean; updateQuestion: (id: string, patch: Partial<EnglishQuestion>) => void; remove: () => void }) {
  const inlineSchool = usesInlineSchoolChoices(set, question)
  const inlinePosition = isInlinePositionTemplate(question.csatTemplateId) || inlineSchool
  const inlineLabels = inlineSchool ? schoolInlineChoiceLabels(set, question) : ['①', '②', '③', '④', '⑤']
  ...
  return <details className="question-editor editor-fold">
    ...
    <div className="editor-fold-body">
      <div className="form-grid"><label>문항 유형<small className="field-help">학생에게 요구할 풀이 방식과 문제 구조를 정합니다. 유형을 바꾸면 기본 발문과 선지 조판도 함께 바뀝니다.</small><select value={question.type} disabled={locked} onChange={(event) => { const type = event.target.value; const template = set.mode === 'school' ? SCHOOL_QUESTION_TEMPLATES.find((candidate) => candidate.questionType === type) : undefined; const stemLanguage = question.schoolStemLanguage ?? 'ko'; const languageAware = schoolQuestionUsesChoiceLanguage({ type, schoolTemplateId: template?.id }); updateQuestion(question.id, { type, stem: defaultQuestionStem(type, stemLanguage), schoolTemplateId: template?.id, schoolChoiceLayout: template?.choiceLayout, schoolStemLanguage: stemLanguage, schoolChoiceLanguage: languageAware ? template?.id === 'summary' ? 'en' : stemLanguage : undefined, schoolSummaryText: template?.id === 'summary' ? question.schoolSummaryText ?? '' : undefined }) }}>{locked ? <option>{question.type}</option> : questionTypesFor(set.mode).map((type) => <option key={type}>{type}</option>)}</select></label><label>배점<small className="field-help">시험지에 표시할 문항 점수입니다. 소수 배점도 입력할 수 있습니다.</small><input type="number" min="0.1" max="100" step="0.1" value={question.score ?? 2} onChange={(event) => updateQuestion(question.id, { score: Number(event.target.value) })} /></label>{set.mode === 'school' && <label>선지 표시<small className="field-help">자동은 선지 길이에 따라 한 줄·세로형을 결정합니다. 조합형은 표 형태가 적합합니다.</small><select value={question.schoolChoiceLayout ?? inferSchoolQuestionTemplate(question).choiceLayout} onChange={(event) => updateQuestion(question.id, { schoolChoiceLayout: event.target.value as EnglishQuestion['schoolChoiceLayout'] })}><option value="auto">길이에 따라 자동</option><option value="inline">짧은 선지 · 한 줄/복수 열</option><option value="vertical">긴 선지 · 세로형</option><option value="matrix">조합형 · 표 형태</option></select></label>}</div>
      {set.mode === 'school' && !locked && <div className="form-grid"><label>{inferSchoolQuestionTemplate(question).id === 'summary' ? '발문 언어' : '문항·선지 언어'}<small className="field-help">{inferSchoolQuestionTemplate(question).id === 'summary' ? '발문은 선택한 언어를 따르고 요약문과 (A)·(B) 단어쌍은 영어로 고정됩니다.' : '발문과 내용 선지를 한국어 또는 영어 중 하나로 함께 통일합니다. 위치 기호는 그대로 유지됩니다.'}</small><select value={question.schoolStemLanguage ?? 'ko'} onChange={(event) => { const language = event.target.value as ProvidedPassageChoiceLanguage; updateQuestion(question.id, { schoolStemLanguage: language, schoolChoiceLanguage: schoolQuestionUsesChoiceLanguage(question) ? inferSchoolQuestionTemplate(question).id === 'summary' ? 'en' : language : undefined, stem: defaultQuestionStem(question.type, language) }) }}>{(Object.keys(PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS) as ProvidedPassageChoiceLanguage[]).map((value) => <option key={value} value={value}>{PROVIDED_PASSAGE_CHOICE_LANGUAGE_LABELS[value]}</option>)}</select></label></div>}
      <label>발문<small className="field-help">시험지에서 학생이 실제로 읽는 질문 문장입니다. 표식형 문항은 사용할 기호 범위를 자동으로 표시합니다.</small><textarea value={set.mode === 'school' ? schoolQuestionDisplayStem(set, question) : question.stem} readOnly={inlineSchool} onChange={(event) => updateQuestion(question.id, { stem: event.target.value })} /></label>
      {set.mode === 'school' && inferSchoolQuestionTemplate(question).id === 'summary' && <label>요약문<small className="field-help">공통 지문 아래 별도 네모 상자에 표시됩니다. (A)와 (B) 자리에 [[요약빈칸:A]], [[요약빈칸:B]]를 각각 입력하세요.</small><textarea value={question.schoolSummaryText ?? ''} placeholder="The passage explains that [[요약빈칸:A]] can [[요약빈칸:B]] learning." onChange={(event) => updateQuestion(question.id, { schoolSummaryText: event.target.value })} /></label>}
      {inlinePosition ? <p className="inline-position-note">이 유형은 별도의 내용 선지를 작성하지 않습니다. 정답은 지문 안 {inlineLabels.join('·')} 표식 중에서 선택합니다.</p> : <>{embeddedChartChoices && <p className="inline-position-note">25번형의 각 칸에는 번호를 제외한 영어 완전문장을 입력하세요. 시험지에서는 도표 아래 설명문에 ①~⑤ 진술로 이어서 표시됩니다.</p>}<div className="choice-grid">{question.choices.map((choice, choiceIndex) => <label key={choiceIndex}><span>{embeddedChartChoices ? ['①', '②', '③', '④', '⑤'][choiceIndex] : choiceIndex + 1}</span><input value={choice} placeholder={embeddedChartChoices ? '지문 속 영어 진술' : '비워두면 AI가 작성'} onChange={(event) => { const choices = [...question.choices]; choices[choiceIndex] = event.target.value; updateQuestion(question.id, { choices }) }} /></label>)}</div></>}
      <div className="form-grid"><label>정답 번호<small className="field-help">현재 선지 또는 지문 위치 중 유일한 정답을 선택합니다.</small><select value={question.answerIndex} onChange={(event) => updateQuestion(question.id, { answerIndex: Number(event.target.value) })}>{question.choices.map((_, choiceIndex) => <option value={choiceIndex + 1} key={choiceIndex}>{inlinePosition ? `지문 위치 ${inlineLabels[choiceIndex]}` : embeddedChartChoices ? `지문 속 ${['①', '②', '③', '④', '⑤'][choiceIndex]}` : `${choiceIndex + 1}번`}</option>)}</select></label><label>문항별 출제 의도<small className="field-help">이 문항이 평가하는 구체적인 사고 능력을 기록합니다.</small><input value={question.intention} onChange={(event) => updateQuestion(question.id, { intention: event.target.value })} /></label></div>
      <label>상세 해설<small className="field-help">정답이 되는 이유와 필요한 독해 과정을 설명합니다. 이 문항의 위치 기호는 {inlineLabels.join('·')}입니다.</small><textarea value={displayText(question.explanation)} onChange={(event) => updateQuestion(question.id, { explanation: event.target.value })} /></label>
      <div className="form-grid"><label>정답 근거 <small className="field-help">지문에서 정답을 확정하는 문장이나 정보를 한 줄에 하나씩 적습니다.</small><textarea value={displayText(question.evidenceRefs.join('\n'))} onChange={(event) => updateQuestion(question.id, { evidenceRefs: event.target.value.split('\n').map((item) => item.trim()).filter(Boolean) })} /></label><label>오답 오류 근거 <small className="field-help">각 오답이 틀린 이유를 선지 순서에 맞춰 한 줄씩 적습니다. 이 문항의 위치 기호는 {inlineLabels.join('·')}입니다.</small><textarea value={displayText(question.distractorReasons.join('\n'))} onChange={(event) => updateQuestion(question.id, { distractorReasons: event.target.value.split('\n').map((item) => item.trim()).filter(Boolean) })} /></label></div>
    </div>
  </details>
}

function createExam(preset: LayoutPreset = 'school-exam'): EnglishExamDocument {
