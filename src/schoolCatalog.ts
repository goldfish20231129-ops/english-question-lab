...
export const SCHOOL_QUESTION_TEMPLATES: readonly SchoolQuestionTemplateDefinition[] = [
  ...
  { id: 'summary', label: '요약문 완성', questionType: '요약문 완성', defaultStem: '다음 글의 내용을 한 문장으로 요약하고자 한다. 빈칸에 들어갈 말로 가장 적절한 것은?', choiceLayout: 'auto', promptRule: '원문과 별도 요약문을 만들고 요약문에 [[요약빈칸:A]] 또는 [[빈칸]]을 둔다.' },
...
export function validateSchoolTemplateMarkup(set: EnglishQuestionSet) {
  ...
  set.questions.forEach((question, index) => {
    ...
    if (template.id === 'summary' && !/\[\[(?:요약빈칸|빈칸)(?::[^\]]+)?\]\]/.test(set.materialSpec?.kind === 'summary' ? set.materialSpec.summary : set.material)) issues.push(`${prefix}: 요약문 빈칸 표식이 필요합니다.`)
