...
`summary`는 권위 원문을 근거로 `question.summaryText`에 별도의 자연스러운 영어 요약문 한 문장을 둔다. JSON에서는 원문을 반복하지 않지만 시험지 조판에서는 일반 문항용 공통 지문과 별개로 같은 원문을 요약 문항 앞에 다시 출력한다. `[[요약빈칸:A]]`와 `[[요약빈칸:B]]`는 각각 정확히 한 번이어야 하며, 다섯 choices는 `A단어|B단어` 형식의 단어쌍이다. 각 choice의 `|`는 정확히 하나이고 양쪽 값은 비어 있지 않아야 한다. 정답 하나만 두 빈칸을 문법적·의미적·논리적으로 모두 충족한다.
